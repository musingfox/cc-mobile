...
這道閘只關了兩扇門中的一扇。#29 之後、問題卡之前的 bundle 右滑送的是 `optionId`，在 wire 上與
人手點擊完全同形，server 分不出來，所以那條路仍然會答出第一個選項——直到手機重新載入、卡片
不再接受手勢為止。**這是已接受的殘留風險**，不是漏掉的檢查：為一個過渡期 bundle 在協定上加
「這是手勢還是點擊」只會把成本留在協定裡。
