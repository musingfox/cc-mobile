...
- **zellij**：原 spike 時未安裝；2026-06-17 應人類要求 `brew install zellij` 裝上 **0.44.3**，補跑對等實測(見「Spike Z：zellij 對等實測」節)。原各 spike 節中「zellij 臂膀 SKIP」已由真實證據取代。
...
- **zellij**：`zellij action write-chars "<text>"` 後 `zellij action write 13`(Enter)——**兩道命令**(tmux 一道 `send-keys ... Enter`)。**實測修正(Spike Z-2)**:特殊字元(`"`, `$`, `|`, `;`, `` ` ``)字面通透、**無需 escape**——先前憑文件臆測「需 escape」有誤。注意 `write 27`(ESC)取代失效的 `send-keys "Escape"`。
...
- **zellij**：`zellij action dump-screen <file>` — 類似 capture-pane。**實測(Spike Z-1)**:外部呼叫須 ① 用 `ZELLIJ_SESSION_NAME=<name>` env var 定位 session(無 `--session <name>` flag)② 先 `list-panes` 取得 pane id(如 `terminal_0`)再 `dump-screen --pane-id`。比 tmux `capture-pane -t <session>` 多兩道手續。
...
- **zellij**：2021 年起，Rust 生態，0.44.3(2026-06-17 補裝實測)，macOS 需 `brew install zellij`（非預裝）。相依性高於 tmux，成熟度尚低於 tmux。

**結論**：tmux 在相依性與成熟度上均優於 zellij，且 tmux 的 send-keys / capture-pane / pipe-pane 功能集完整覆蓋 C-hybrid 所需。

---

## Spike Z：zellij 0.44.3 對等實測（2026-06-17 補測，取代原 SKIP）

應人類要求裝上 zellij 0.44.3，對 C-hybrid 三項核心需求跑與 tmux 對等的 live spike（真機、真 claude 2.1.179）。腳本＋真實擷取輸出：`docs/adr/spike-014/spike-014z-{1,2,3}-*.{sh,txt}`。

- **Z-1 dump-screen 讀畫面 / 就緒：PASS** — `ZELLIJ_SESSION_NAME` 定位 + `list-panes` 取 pane id + `dump-screen --pane-id`，1000ms 內讀到就緒 token。可行但比 `capture-pane -t` 多兩道手續。（證據：`spike-014z-1-dumpscreen.txt`）
- **Z-2 write-chars 注入：PASS** — 特殊字元字面通透、**無需 escape**（修正先前文件臆測）；提交需 `write-chars` + `write 13` 兩道。（證據：`spike-014z-2-writechars.txt`，含 `echo "hello $USER | grep test; ls \`pwd\`"` 注入落地）
- **Z-3 Stop hook 在 zellij session 內觸發：PASS** — `claude --settings` 載入 hook，4500ms 內觸發，marker file 含完整 JSON payload，行為與 tmux 臂膀一致（hook 為 multiplexer 無關，已實證）。（證據：`spike-014z-3-hook.txt`）

**淨裁決：WORSE-but-feasible。** zellij 三項全 PASS，C-hybrid 在 zellij 上技術可行、且可移植（若目標機僅有 zellij）。但有兩個結構性摩擦使 tmux 維持較佳選擇：① 無 `--session <name>` flag，須靠 `ZELLIJ_SESSION_NAME` env var 定位 session；② 每次 action 前須 `list-panes` 取 pane id；③ 提交需兩道命令。tmux 對 headless/scripted 驅動更成熟、更低摩擦。**此實測修訂了原「zellij 僅文件、SKIP」的不對稱——tmux 的選擇現由對等實證支撐，而非單邊推論。**
